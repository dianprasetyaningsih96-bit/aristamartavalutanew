-- Correcting the role name from 'manager' to 'branch_manager' to match the database enum
-- Using idempotent checks (CREATE TABLE IF NOT EXISTS, DROP POLICY IF EXISTS)

-- 1. Ensure is_head_office column exists
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='branches' AND column_name='is_head_office') THEN
        ALTER TABLE public.branches ADD COLUMN is_head_office BOOLEAN DEFAULT FALSE;
    END IF;
END $$;

-- Set Jimbaran as Head Office if not already set
UPDATE public.branches 
SET is_head_office = TRUE 
WHERE name ILIKE '%Jimbaran%';

-- 2. Create branch_transfers table if it doesn't exist
...
-- GRANTs are necessary even if the table exists
