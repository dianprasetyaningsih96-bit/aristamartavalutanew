...
BEGIN
    ...
    -- Get currency code for logging/notifications
    SELECT code INTO v_currency_code FROM public.currencies WHERE id = t_row.currency_id;
    ...
    ELSIF p_status = 'rejected' THEN
        ...
        SELECT 
            p.id, 
            'Transfer Ditolak', 
            'Transfer ' || t_row.amount || ' ' || v_currency_code || ' ditolak oleh Kantor Pusat: ' || COALESCE(p_notes, ''),
            'warning',
            t_row.branch_id,
            'approval_decision',
            '/cash'
